...
int main(int argc, char *argv[])
{
	...
	Hash hashh;
	omp_set_num_threads(opt->thread);
	hashh.write_hashfile(opt->hashdir,genome,len_genome,opt->len_sed,opt->K,opt->L,opt->lambda, opt->zero);
	 time(&time2);
   struct rusage r_usage;
      
	//fprintf(stderr,"%d\t%d\t%d\t%d\t%.2f",opt->thread,opt->lambda,opt->L,opt->zero,difftime(time2, time1));
	fprintf(stderr,"Parameters used...\n Number of threads=%d\n K=%d\n Context size (2*lambda+1) = %d\n L=%d\n zero=%d\n\nIndexing time %.0f sec\n\n\n",opt->thread,opt->K,opt->lambda,opt->L,opt->zero,difftime(time2, time1));
...
//fprintf(stderr,"Peak memory foorprint %ld MB\n",r_usage.ru_maxrss/1000000);
  //fprintf(stderr,"%d\t%.2f\t%.2f\t%ld ",opt->thread,difftime(time2, time1),difftime(time4, time3),r_usage.ru_maxrss/1000);
	
	fprintf(stderr,"Parameters used...\n Number of threads=%d\n Match Score=%d\n Mismatch penalty=%d\n Gap_open penalty=%d\n Gap_extend penalty=%d\n\nAlignment time %.0f sec\nPeak memory foorprint %ld MB\n",opt->thread,opt->match,opt->mismatch,opt->gapopen,opt->gapextend,difftime(time4, time3),r_usage.ru_maxrss/1000000);
