...
int generate_conLSH( uint32_t *h_pos, uint8_t *h_j, uint32_t *hval, uint32_t len_genome, uint32_t len_sed, char *genome, uint32_t L,uint32_t *pat)
{
	...
	uint32_t num_pre;
        ...
        #pragma omp parallel for default(shared) private(i,j, num_pre) reduction(+ : acn)
	for( i = 1; i < len_genome-len_sed; i+=1) {
		...
		//i+=j;
//		if ((i < len_genome - len_sed) && (i<((4+1)*((len_genome-len_sed)/4))))
//              // if (i < len_genome - len_sed) 
//               {  
		
	    // Use only one pattern for genome
		
			num_pre=transfer(genome + i,len_sed,pat[0]);
...
//                        
	...
	//*c=acn-1;
	//cout<<"\t total valid sequences="<<acn-1<<"\n";
	return(acn);
}
	...
	for (uint32_t pp=0;pp<L;pp++) // generate hash values for L different patterns
   {
	...
	for( uint32_t i = 1; i < len_read - 32; i+=tr) {
		...
		if(read[i]=='N'||read[i]=='n')
			...
			num_pre=transfer(read + i,32,pat[pp]);
			if(count<amount)
			{
			  hv[count]=num_pre;
			count++;
...
uint32_t transfer(char *genome,uint32_t len_sed, uint32_t pt)
{
