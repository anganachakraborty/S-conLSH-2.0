...
#pragma omp parallel for default(shared) private(ssn,last,diff)  
	...
	{	
		...
		if(h_index->pos[hashtab->hash_val[ii]]==0)  //first entry of this hash value
	  {
		if(h_index->j[hashtab->hash_val[ii]]!=0 &&  h_index->j[hashtab->hash_val[ii]]!=201)  //valid entry
		{
                                   ...
                                   h_index->pos[hashtab->hash_val[ii]]=next; 
                                 
                                       //starts from 1, subtract 1 at the time of new insertion
