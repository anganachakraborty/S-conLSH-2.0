...
int Aligner::alignFree_SV(kseq_t *trunk, int offset, char *useread, Sam_Rec *_sams,bkt2 *preserved)
{
...
int Aligner::align_SV(kseq_t *trunk, int offset, char *useread, Sam_Rec *_sams,bkt2 *preserved)
{
...
int Aligner::applySV(kseq_t *trunk, Sam_Rec *_svsams, uint32_t L, uint32_t *pat, uint32_t len_genome, int align, int tr)
{
	...
	for (int j=0;j<=1;++j) {
		...
		// for(int ll=start;ll<end && preserved->seq_num[ll]!=0;ll++)
				//cout<<preserved->seq_num[ll]<<"\t"<<preserved->hit_times[ll]<<"\t"<<preserved->isrc[ll]<<endl;
		if(align==0)
			count_aligned=alignFree_NonSV(trunk,useread,_svsams,preserved);
		 else
			count_aligned=align_NonSV(trunk,useread,_svsams,preserved);
...
int Aligner::alignFree_NonSV(kseq_t *trunk, char *useread ,Sam_Rec *_sams,bkt2 *preserved)
{
					uint32_t c_align=0, max[2], chim=0;
					//int msc=0;
					uint32_t canReadLen=trunk->seq.l;
...
int Aligner::align_NonSV(kseq_t *trunk, char *useread ,Sam_Rec *_sams, bkt2 *preserved)
{
...
int  Aligner::applyNonSV(kseq_t *trunk, Sam_Rec *_sams, uint32_t L, uint32_t *pat, uint32_t len_genome, int align, int tr)
{
	...
	for (int j=0;j<=1;j++) {
		...
		//align using KSW 
		if(align==1)
			count_aligned=align_NonSV(trunk, useread, _sams, preserved);
		else
			count_aligned=alignFree_NonSV(trunk, useread, _sams,preserved);	
  ...
  if(algn==1)
  {
	...
	fp = gzopen(readpath, "r");
            ...
            seqs = kseq_init(fp); 
              ...
              int sstop=0,tstop=-1;  
                #pragma omp parallel default(shared) firstprivate(tstop)
            {
                ...
                while(!sstop)
               {
                    ...
                    kseq_t *seqss=kseq_init(fp); ;
                   // seqss=(kseq_t*)calloc(1, sizeof(kseq_t));	 
		   free(seqss->f->buf);
                    #pragma omp critical
                   {
                       seqss->comment=seqs->comment;
                       seqss->f=seqs->f;
                       seqss->last_char=seqs->last_char;
                       
                       tstop=kseq_read(seqss);
                       
                       
                       seqs->comment=seqss->comment;
                       seqs->f=seqss->f;
                       seqs->last_char=seqss->last_char;
                       
                       
                   }
                   if(tstop>=0)
                   {
                       revComRead(seqss->seq.s, seqss->seq.rs, seqss->seq.l);
				uint16_t sam_details;
                                 
//                                    #pragma omp critical
//                                {
//                                    cout<< seqss->name.s <<endl;
//                                }
		
			if(seqss->seq.l <= LEN_BASES)
			{
				sam_details = applyNonSV(seqss, sams, L, pat, len_genome,algn, 16);
				sam_details = applyNonSV(seqss, sams, L, pat, len_genome,algn, 8);
				sam_details = applyNonSV(seqss, sams, L, pat, len_genome,algn, 4);
				

			}
			else if(seqss->seq.l <= 4*LEN_BASES)
				sam_details = applyNonSV(seqss, sams, L, pat, len_genome,algn, 16);
			else if(seqss->seq.l <= 6*LEN_BASES)
				sam_details = applyNonSV(seqss, sams, L, pat, len_genome,algn, 32);
			else if(seqss->seq.l >= 10*LEN_BASES && seqss->seq.l <= 15*LEN_BASES)
				sam_details = applyNonSV(seqss, sams, L, pat, len_genome,algn, 64);
				
				
		if ( !(seqss->seq.l <= LEN_BASES/2)) {
				
				if(seqss->seq.l <= 4*LEN_BASES)
					sam_details = applySV(seqss, sams, L, pat, len_genome,algn,4);
				else if(seqss->seq.l <= 6*LEN_BASES)
					sam_details = applySV(seqss, sams, L, pat, len_genome,algn,8);
				else if(seqss->seq.l <= 10*LEN_BASES)
					sam_details = applySV(seqss, sams, L, pat, len_genome,algn,16);
				else
					sam_details = applySV(seqss, sams, L, pat, len_genome,algn,32);
			} 
                    
                    
                    	 free(seqss->name.s);
			// free(seqss->comment.s); 
			free(seqss->seq.s);	free(seqss->qual.s);
                       free(seqss);
                   }
                   else
                   {
